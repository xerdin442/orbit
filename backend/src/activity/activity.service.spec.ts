...
describe('ActivityService', () => {
  ...
  describe('findLogs', () => {
    ...
    it('filters by exact type', async () => {
      await service.findLogs({ actorId: 'user-1', type: 'project_created' });
      expect(db.activity.findMany).toHaveBeenCalledWith({
        where: { actorId: 'user-1', type: { equals: 'project_created' } },
        orderBy: { createdAt: 'desc' },
      });
    });

    it('filters by wildcard type pattern', async () => {
      await service.findLogs({ actorId: 'user-1', type: 'slack_*' });
      expect(db.activity.findMany).toHaveBeenCalledWith({
        where: { actorId: 'user-1', type: { startsWith: 'slack_' } },
        orderBy: { createdAt: 'desc' },
      });
    });

    it('filters by wildcard type pattern with nested prefix', async () => {
      await service.findLogs({ actorId: 'user-1', type: 'github_*' });
      expect(db.activity.findMany).toHaveBeenCalledWith({
        where: { actorId: 'user-1', type: { startsWith: 'github_' } },
        orderBy: { createdAt: 'desc' },
      });
    });

    it('filters by projectId via metadata path', async () => {
      await service.findLogs({ actorId: 'user-1', projectId: 'proj-1' });
      expect(db.activity.findMany).toHaveBeenCalledWith({
        where: {
          actorId: 'user-1',
    ...
    it('filters by deploymentId', async () => {
      await service.findLogs({ actorId: 'user-1', deploymentId: 'dep-1' });
      expect(db.activity.findMany).toHaveBeenCalledWith({
        where: {
          actorId: 'user-1',
